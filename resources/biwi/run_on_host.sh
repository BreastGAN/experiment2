...
BASE_HOME=/scratch_net/biwidl104/$USER
...
echo "Run model:"
cd $PROJECT_HOME
base_path="/scratch_net/biwidl104/$USER/transformed/small_all_512x408_final"
exec python -m models.breast_cycle_gan.train --height 512 --width 408 --image_x_file "$base_path"/healthy.train.tfrecord --image_y_file "$base_path"/cancer.train.tfrecord --include_masks="$masks" --train_log_dir="data_out/$exp_name" --use_icnr="$icnr" --upsample_method="$upsample_method" --loss_identity_lambda="$loss_identity_lambda" --checkpoint_hook_steps="$checkpoint_hook_steps" --use_spectral_norm="$spectral_norm"
